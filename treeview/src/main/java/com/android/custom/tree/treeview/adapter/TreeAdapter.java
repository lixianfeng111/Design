            ...
            public void onClick(View v) {
                if (finalChildView != null) {
                    if (finalChildView.getVisibility() == VISIBLE) {
                        ...
                        holder.ivLeft.setImageResource(R.drawable.ic_org_unexpand);
                    } else {
                        finalChildView.setVisibility(VISIBLE);
                        holder.ivLeft.setImageResource(R.drawable.ic_org_expanded);
                    }
                }
            }
        });
...
//                if (title.contains("图册分类")||title.contains("第04篇")||title.contains("第01册")||title.contains("第02册")){
//
//                }else {
//                    EventBus.getDefault().post(new EmptyEvent(title));
//                }
                if (title.contains("专项设计")||title.contains("车站建筑")){
                    EventBus.getDefault().post(new EmptyEvent(title));
                }
